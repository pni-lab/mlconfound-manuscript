...
The proposed \emph{partial} and \emph{full confoudner tests} for predictive modelling have been implemented in the python package \emph{mlconfound}\footnote{\href{https://mlconfound.readthedocs.io}{https://mlconfound.readthedocs.io}}. The inner workings of the tests is summarized on Fig. \ref{fig:overview}. In short, the tests model the conditional distribution between the confounder and the target variable with a generalized additive model (GAM) and then use a parallel-pairwise Markov-chain Monte-Carlo sampler that draws permutations of the confounder or target variable, for the partial and full confounder test, respectively. As a result, the permuted variables comply with the GAM-based conditional distribution. The test statistic (coefficient of determination, $R^2$) is then computed between the model predictions and the original, as well as the permuted variables. The original and the permuted test statistics construct the p-value as the ratio of permuted test statistics more extreme than the original. The strength of this approach lies in avoiding to place any assumptions on the conditional distributions of the model output. This property ensures valid model diagnostics even in cases where Pearson and Spearman partial correlation and many other methods fail (as shown on Fig. \ref{fig:sim-h0-demo}).

\subsection{Simulations}

\subsubsection*{Type I error}

As suggested by theory and shown by the simulations, both of the proposed tests provide a valid Type I error control (Fig. \ref{fig:sim-normal}), even in case of non-linearity and non-normality (Figs. \ref{fig:sim-h0-demo} and \ref{fig:sim-non-normal}), except when non-normality is very extreme (purple distribution on Fig. \ref{fig:sim-non-normal}, kurtosis: 42, skewness: -6). With this the proposed tests overcome the limitations of partial Spearman correlation and linear conditional permutations, demonstrated on Fig. \ref{fig:sim-h0-demo}.

For detailed results, see also Supplementary Figures \ref{fig:sim-ccc-sig-partial}, \ref{fig:sim-bbb-lin-partial}, \ref{fig:sim-bbb-sig-partial}, \ref{fig:sim-ccc-sig-full}, \ref{fig:sim-bbb-lin-full} and \ref{fig:sim-bbb-sig-full}.
...
Confounder-bias can significantly harm the generalizability and biomedical validity of predictive models. The problem of testing confounder-bias in predictive models can considered as assessing \emph{conditional independence} between the predicted values, the the target and the confounder variable.
Importantly, in predictive modelling, the model predictions are often characterized by a non-normal and/or non-linear conditional dependence on the target variable, thus any test for confounder bias must be, at least partly, non-parametric.
However, as shown by \citet{shah2020hardness} in their 'no free lunch' theorem, it is effectively impossible to establish a fully non-parametric conditional independence test with a valid type I error control and non-trivial power. In practice, this means that all candidate tests, that assume normality and/or linearity of the joint distribution, will necessarily fail to properly control for Type I errors in many realistic predictive modelling scenarios. This is clearly demonstrated on Fig. \ref{fig:sim-h0-demo} in case of partial Spearman correlation. Although warnings about these issues were given from early on \citep{korn1984ranges} and received a fair amount of attention recently \citep{bergsma2010nonparametric, candes2016panning, peters2016causal,  shah2020hardness, berrett2020conditional}, the magnitude of the problem may not be fully appreciated within the predictive modelling community.

From the three involved variables, the most problematic one is clearly the prediction output, known to often display non-normal and non-linear conditional dependencies on the other two. Therefore, a method being distribution-free for this variable, but inputting custom distributional information about the other two variables might provide a sufficient flexibility required for predictive model diagnostics.

To achieve this, here I propose to combine the conditional permutation test (CPT) of \cite{berrett2020conditional} with generalized additive model (GAM)-based conditional distribution modelling \citep{hastie1987generalized}. The proposed mathematical framework offers two novel tests for probing confounder bias: the partial and full confounder tests. These tests can handle both numerical and categorical data and can be applied for any predictive model, without having to re-fit the model, that is, with a negligible extra computational cost.

As expected from theory, both tests displayed a valid type I error control in the simulations, even if both the predictions and the confounder are non-normally and/or non-linearly dependent on the target variable.
Even though the tests' valid type I error control is justified by theory and reinforced by the simulations, a low statistical power could significantly limit their usefulness. The simulations presented in this paper revealed that both tests have a high, practically relevant power in a wide variety of scenarios. Considering the partial test, if we apply \citeauthor{cohen2013statistical}'s terminology for the effect size of the confounder bias, sample sizes around 500-1000 can robustly identify a very weak confounder bias (e.g a confounder explaining about 4\% of the variance in the prediction). A lower sample size of 100 will still very likely identify medium sized confounder-biases (around 12\%). Strong biases  (>25\%) can be very robustly detected with sample sizes as low as 50. As expected, the power of the full confounder test is very similar to that of the partial confounder test, since the only difference is that the conditional contribution of the target to the prediction is investigated, instead that of the confounder.
While different biomedical applications might consider different amounts of bias to be relevant, an a-priori power calculation can help identifying the necessary sample size for proper model diagnostics.

Simulations also confirmed that non-normality and non-linearity of the conditional distribution of the predictions and the confounder (given the target) has a negligible effect on the power of both tests, except in case of extreme violations of normality. This result confirms that the proposed tests, unlike e.g. partial correlation, remain valid, independent of the distributional characteristics of the applied model's output.
...
In the ABIDE dataset, neither the medium-strong center-bias nor the age-bias is surprising but both are obviously problematic for a diagnostic model of ASD. For instance the model trained on the raw (unadjusted) features - depending on the calibration of the predicted class probabilities - might classify all participants form e.g. the CMU (Carnegie Mellon University) center as neurotypical control participants. It is easy to see that this bias can lead to calibration issues and, possibly, a very low predictive performance on out-of-sample data. Similarly, a model biased by motion - next to having questionable neuroscientific validity - might systematically fail in populations with a tendency for higher in-scanner motion, e.g. due to atypical motor function (as known e.g. for ADHD \citep{eloyan2012automated}).

Confounder mitigation techniques, with one exception, tended to decrease confounder bias. The partial confounder test provided quantitative, statistically rigorous metrics for assessing the effectiveness of the techniques for each confounder. In the HCP data, it revealed that both the acquisition-bias and the age-bias was very effectively removed by both feature regression and COMBAT (p>0.05 for all). Given the high power of the test at the sample sizes of the HCP dataset ($N=999$), any remaining confounder-bias is most probably very safely negligible and well out of the range of practical relevance.
...
This example highlight that the full confounder test might become useful in exploratory phases of model development, where models might be still severely biased by various confounders and the questions is whether there is any biomedically relevant signal captured by the model, at all.
