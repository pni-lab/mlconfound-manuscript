...
The lack of rigorous non-parametric statistical tests of confoudner-effects significantly hampers the development of robust, valid ans generalizable predictive models in many fields of research.
Here I propose the \emph{partial} and \emph{full confounder tests}, that build on a recent theoretical framework of conditional independence testing and test the null hypothesis of \emph{no bias} and \emph{fully biased model}, respectively.
The proposed tests set no assumptions about the distribution of the predictive model output that is often non-normal.
As shown by simulations, the test are statistically valid, robust and display a high statistical power.
Applying the tests on models based on functional brain connectivity data from the Human Connectome Project and the Autism Brain Imaging Data Exchange databases shows that the proposed tests can significantly improve the assessment of the robustness, generalizability and neurobiological validity of predictive models and, thereby, foster the development of clinically useful machine learning biomarkers.

The tests are implemented in the python package \emph{mlconfound}\footnote{https://github.com/pni-lab/mlconfound}.
\end{abstract}
...
\end{tabular}
\caption{\label{tab:unconditional-pvals} Unconditioned coefficients-of-determination and the corresponding p-values and the p-values of the partial and full confounder tests for all investigated models.}
\end{table}
